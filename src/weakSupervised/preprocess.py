...
df = pd.read_csv(os.path.join(dataDir, 'kiba-smiles.csv'))
...
for i in range(len(smiles)):
    result = pool.apply_async(get_properties_mol, (smiles[i], ))
...
for idx, result in enumerate(results):
    if (idx+1) % 10000 == 0:
        ...
        inter_df.to_csv(os.path.join(dataDir, "kiba_props_{}.csv".format(idx)), index=False, header=True)
        props_intermediate = list()
